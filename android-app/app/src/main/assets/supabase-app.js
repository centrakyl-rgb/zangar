(function(){
  ...
  async function addSchedulePanel(canEdit,onlyMine){
    ...
    var list=rows.map(function(x){return '<div class="item schedule-row" data-weekday="'+x.weekday+'"><div class="row"><strong>'+days[x.weekday]+' · '+String(x.lesson_time).slice(0,5)+'</strong><span class="badge">'+esc(x.subjects&&x.subjects.name)+'</span></div><p>'+esc(x.groups&&x.groups.name)+' · '+esc(x.profiles&&x.profiles.full_name)+'</p>'+(canEdit?'<div class="row" style="margin-top:9px"><button type="button" class="btn" data-schedule-edit="'+x.id+'">Изменить</button><button type="button" class="btn" data-schedule-delete="'+x.id+'">Удалить</button></div>':'')+'</div>'}).join("")||'<div class="empty">Расписание пока не заполнено</div>';
    var form="";
    ...
    if(canEdit&&$("scheduleForm")){
      $("scheduleForm").onsubmit=async function(e){e.preventDefault();var id=$("scheduleId").value,payload={weekday:Number($("scheduleDay").value),lesson_time:$("scheduleTime").value,group_id:$("scheduleGroup").value,subject_id:$("scheduleSubject").value,teacher_id:$("scheduleTeacher").value};try{await api("/rest/v1/schedule_entries"+(id?"?id=eq."+id:""),{method:id?"PATCH":"POST",headers:{"Prefer":"return=minimal"},body:JSON.stringify(payload)});await render()}catch(err){alert(err.message)}};
