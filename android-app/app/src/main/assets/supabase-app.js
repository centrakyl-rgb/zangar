(function(){
  ...
  async function addSchedulePanel(canEdit,onlyMine){
    ...
    var list=rows.map(function(x){return '<div class="item schedule-row" data-weekday="'+x.weekday+'"><div class="row"><strong>'+days[x.weekday]+' · '+periodLabel(x.lesson_time)+'</strong><span class="badge">'+esc(x.subjects&&x.subjects.name)+'</span></div><p>'+esc(x.groups&&x.groups.name)+' · '+esc(x.profiles&&x.profiles.full_name)+'</p>'+(canEdit?'<div class="row" style="margin-top:9px"><button type="button" class="btn" data-schedule-edit="'+x.id+'">Изменить</button><button type="button" class="btn" data-schedule-delete="'+x.id+'">Удалить</button></div>':'')+'</div>'}).join("")||'<div class="empty">Расписание пока не заполнено</div>';
    var form="";
    ...
    var filter='<label>Показать день</label><select id="scheduleFilter"><option value="all">Вся неделя</option>'+days.slice(1).map(function(d,i){return '<option value="'+(i+1)+'">'+d+'</option>'}).join('')+'</select>';
    var section=document.createElement("section");section.className="panel";section.innerHTML='<h3>Расписание</h3>'+form+filter+'<div class="list" style="margin-top:12px">'+list+'</div>';$("dash").appendChild(section);
    $("scheduleFilter").onchange=function(){var day=this.value;section.querySelectorAll(".schedule-row").forEach(function(row){row.classList.toggle("hidden",day!=="all"&&row.dataset.weekday!==day)})};
