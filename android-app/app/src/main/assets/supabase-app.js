(function(){
  ...
  async function saveQuickClass(e,choices){
    ...
    try{
      var rows=Array.from(document.querySelectorAll(".quickStatus")).map(function(s){var x=choices[Number(s.dataset.i)],status=s.value;return {lesson_date:today,teacher_id:user.id,student_id:x.student.id,subject_id:x.subject.id,attendance:status==="ill"?"ill":status==="absent"?"absent":"present",lesson_status:status,mistakes:0,comment:status==="unprepared"?"Не подготовил урок":null}});
