(function(){
  ...
  $("loginForm").onsubmit=async function(e){
    ...
    try{await login(email,password);$("passwordOnce").classList.add("hidden");$("password").value=""}
    catch(err){$("passwordOnce").classList.remove("hidden");$("error").textContent="Введите пароль для этого сотрудника"}
  };
  ...
  async function changeOwnPassword(e){
    ...
    try{await api("/auth/v1/user",{method:"PUT",body:JSON.stringify({password:first})});alert("Пароль изменён");e.target.reset()}
  ...
  async function addSchedulePanel(canEdit,onlyMine){
    ...
    var rows=await api(query),days=["","Понедельник","Вторник","Среда","Четверг","Пятница","Суббота"],periods=[{n:1,start:"08:00",end:"08:45"},{n:2,start:"08:55",end:"09:40"},{n:3,start:"09:50",end:"10:35"},{n:4,start:"10:45",end:"11:30"},{n:5,start:"11:40",end:"12:25"},{n:6,start:"12:35",end:"13:20"},{n:7,start:"13:30",end:"14:15"},{n:8,start:"14:25",end:"15:10"}];if(window.AkylNative&&window.AkylNative.scheduleNotifications)window.AkylNative.scheduleNotifications(JSON.stringify(rows));
    function periodLabel(time){var value=String(time).slice(0,5),p=periods.find(function(x){return x.start===value});return p?p.n+'-й урок · '+p.start+'–'+p.end:value}
    function lessonRow(x){return '<div class="item schedule-row"><div class="row"><strong>'+periodLabel(x.lesson_time)+'</strong><span class="badge">'+esc(x.subjects&&x.subjects.name)+'</span></div><p>'+esc(x.groups&&x.groups.name)+' · '+esc(x.profiles&&x.profiles.full_name)+'</p>'+(canEdit?'<div class="row" style="margin-top:9px"><button type="button" class="btn" data-schedule-edit="'+x.id+'">Изменить</button><button type="button" class="btn" data-schedule-delete="'+x.id+'">Удалить</button></div>':'')+'</div>'}
    var list=days.slice(1).map(function(day,index){var weekday=index+1,lessons=rows.filter(function(x){return Number(x.weekday)===weekday});return '<div class="schedule-day-block" data-weekday="'+weekday+'"><h3 style="margin:18px 0 9px">'+day+'</h3>'+(lessons.length?lessons.map(lessonRow).join(''):'<div class="empty">Уроков нет</div>')+'</div>'}).join('');
    var form="";
    if(canEdit){var groups=await api("/rest/v1/groups?active=eq.true&select=id,name&order=name"),subjects=await api("/rest/v1/subjects?select=id,name&order=name"),teachers=await api("/rest/v1/profiles?role=eq.teacher&active=eq.true&select=id,full_name&order=full_name");form='<form id="scheduleForm" class="grid"><input id="scheduleId" type="hidden"><div><label>День</label><select id="scheduleDay">'+days.slice(1).map(function(d,i){return '<option value="'+(i+1)+'">'+d+'</option>'}).join('')+'</select></div><div><label>Номер урока</label><select id="schedulePeriod">'+periods.map(function(p){return '<option value="'+p.start+'">'+p.n+'-й урок · '+p.start+'–'+p.end+'</option>'}).join('')+'<option value="custom">Другое время</option></select></div><div id="scheduleTimeField" class="full hidden"><label>Своё время начала</label><input id="scheduleTime" type="time" value="08:00" required></div><div><label>Группа</label><select id="scheduleGroup">'+groups.map(function(g){return '<option value="'+g.id+'">'+esc(g.name)+'</option>'}).join('')+'</select></div><div><label>Предмет</label><select id="scheduleSubject">'+subjects.map(function(s){return '<option value="'+s.id+'">'+esc(s.name)+'</option>'}).join('')+'</select></div><div class="full"><label>Учитель</label><select id="scheduleTeacher">'+teachers.map(function(t){return '<option value="'+t.id+'">'+esc(t.full_name)+'</option>'}).join('')+'</select></div><button id="scheduleSave" class="btn green">Добавить урок</button><button id="scheduleCancel" type="button" class="btn hidden">Отмена</button></form>'}
    var filter='<h3 style="margin-top:18px">Недельное расписание</h3><label>Показать</label><select id="scheduleFilter"><option value="all">Вся неделя: понедельник–суббота</option>'+days.slice(1).map(function(d,i){return '<option value="'+(i+1)+'">Только '+d.toLowerCase()+'</option>'}).join('')+'</select><button id="printSchedule" type="button" class="btn" style="width:100%;margin-top:10px">Печать недельного расписания</button>';
    var section=document.createElement("section");section.className="panel";section.innerHTML='<h3>Расписание</h3>'+form+filter+'<div class="list schedule-week" style="margin-top:12px">'+list+'</div>';$("dash").appendChild(section);
    $("scheduleFilter").onchange=function(){var day=this.value;section.querySelectorAll(".schedule-day-block").forEach(function(block){block.classList.toggle("hidden",day!=="all"&&block.dataset.weekday!==day)})};
    $("printSchedule").onclick=function(){$("scheduleFilter").value="all";$("scheduleFilter").onchange();window.print()};
    if(canEdit&&$("scheduleForm")){
      $("schedulePeriod").onchange=function(){var custom=this.value==="custom";$("scheduleTimeField").classList.toggle("hidden",!custom);if(!custom)$("scheduleTime").value=this.value};$("scheduleTime").value=periods[0].start;
      $("scheduleForm").onsubmit=async function(e){e.preventDefault();var id=$("scheduleId").value,time=$("schedulePeriod").value==="custom"?$("scheduleTime").value:$("schedulePeriod").value,payload={weekday:Number($("scheduleDay").value),lesson_time:time,group_id:$("scheduleGroup").value,subject_id:$("scheduleSubject").value,teacher_id:$("scheduleTeacher").value};try{await api("/rest/v1/schedule_entries"+(id?"?id=eq."+id:""),{method:id?"PATCH":"POST",headers:{"Prefer":"return=minimal"},body:JSON.stringify(payload)});await render()}catch(err){alert(err.message)}};
      $("scheduleCancel").onclick=function(){$("scheduleForm").reset();$("scheduleId").value="";$("scheduleSave").textContent="Добавить урок";this.classList.add("hidden")};
      section.querySelectorAll("[data-schedule-edit]").forEach(function(b){b.onclick=function(){var x=rows.find(function(r){return String(r.id)===b.dataset.scheduleEdit});if(!x)return;$("scheduleId").value=x.id;$("scheduleDay").value=x.weekday;var time=String(x.lesson_time).slice(0,5),known=periods.some(function(p){return p.start===time});$("schedulePeriod").value=known?time:"custom";$("scheduleTime").value=time;$("scheduleTimeField").classList.toggle("hidden",known);var group=groups.find(function(g){return g.name===(x.groups&&x.groups.name)}),subject=subjects.find(function(s){return s.name===(x.subjects&&x.subjects.name)}),teacher=teachers.find(function(t){return t.full_name===(x.profiles&&x.profiles.full_name)});if(group)$("scheduleGroup").value=group.id;if(subject)$("scheduleSubject").value=subject.id;if(teacher)$("scheduleTeacher").value=teacher.id;$("scheduleSave").textContent="Сохранить изменения";$("scheduleCancel").classList.remove("hidden");$("scheduleForm").scrollIntoView({behavior:"smooth",block:"start"})}});
      section.querySelectorAll("[data-schedule-delete]").forEach(function(b){b.onclick=async function(){if(!confirm("Удалить этот урок из расписания?"))return;try{await api("/rest/v1/schedule_entries?id=eq."+b.dataset.scheduleDelete,{method:"DELETE",headers:{"Prefer":"return=minimal"}});await render()}catch(err){alert(err.message)}}});
    }
  }
  ...
  async function addPrintButton(title){var section=document.createElement("section");section.className="panel";section.innerHTML='<h3>Отчёты и печать</h3><button class="btn green" style="width:100%">Печать / сохранить PDF: '+esc(title)+'</button>';section.querySelector("button").onclick=function(){window.print()};$("dash").appendChild(section)}
  ...
  function makeSectionsMenu(){
    ...
    var panels=Array.from(dash.querySelectorAll(":scope > .panel")),managementTitles=/^(Предметы учителей|Заместитель по учебной части|Добавить сотрудника|Новые сотрудники|Безопасность)$/i,management=panels.filter(function(panel){var h=panel.querySelector(":scope > h3");return h&&managementTitles.test(h.textContent.trim())});
    if(management.length){var combined=document.createElement("section"),combinedHeading=document.createElement("h3");combined.className="panel";combinedHeading.textContent="Сотрудники и доступ";combined.appendChild(combinedHeading);management.forEach(function(panel){var h=panel.querySelector(":scope > h3"),details=document.createElement("details"),summary=document.createElement("summary"),inside=document.createElement("div");details.className="item";summary.textContent=h.textContent.trim();inside.style.marginTop="12px";Array.from(panel.childNodes).forEach(function(node){if(node!==h)inside.appendChild(node)});details.appendChild(summary);details.appendChild(inside);combined.appendChild(details);panel.remove()});dash.appendChild(combined);panels=Array.from(dash.querySelectorAll(":scope > .panel"))}
  ...
  function setupDashboardNavigation(panels,hint){
    function category(title){
      ...
      if(/журнал|запис|сдач|успеваем|Сравнение|Итог сегодняшнего дня|Новая запись|Отч[её]т|печать/i.test(title))return "journal";
